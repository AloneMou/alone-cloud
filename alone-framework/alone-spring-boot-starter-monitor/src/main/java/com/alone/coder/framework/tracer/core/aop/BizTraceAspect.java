package com.alone.coder.framework.tracer.core.aop;

import cn.hutool.core.map.MapUtil;
import cn.hutool.core.util.StrUtil;
import com.alone.coder.framework.common.util.spring.SpringExpressionUtils;
import com.alone.coder.framework.tracer.core.annotation.BizTrace;
import com.alone.coder.framework.tracer.core.util.TracerFrameworkUtils;
...
public class BizTraceAspect {
	...
	private String getOperationName(ProceedingJoinPoint joinPoint, BizTrace trace) {
		...
		// 默认操作名，使用方法名
		return BIZ_OPERATION_NAME_PREFIX + joinPoint.getSignature().getDeclaringType().getSimpleName() + "/"
				+ joinPoint.getSignature().getName();
	}

	private void setBizTag(Span span, ProceedingJoinPoint joinPoint, BizTrace trace) {
		try {
			Map<String, Object> result = SpringExpressionUtils.parseExpressions(joinPoint,
