...
package com.alone.coder.framework.common.util.res;
...
import static com.alone.coder.framework.common.exception.enums.GlobalErrorCodeConstants.SUCCESS;
...
public class RetOps<T> {
	...
	/** 状态码为成功 */
	public static final Predicate<CommonResult<?>> CODE_SUCCESS = r -> Objects.equals(SUCCESS.getCode(), r.getCode());

	/** 数据有值 */
	public static final Predicate<CommonResult<?>> HAS_DATA = r -> ObjectUtil.isNotEmpty(r.getData());

	/** 数据有值,并且包含元素 */
	public static final Predicate<CommonResult<?>> HAS_ELEMENT = r -> ObjectUtil.isNotEmpty(r.getData());
	...
	public boolean isSuccess() {
		return codeEquals(SUCCESS.getCode());
	}
	 ...
	 * @throws Ex 断言失败时抛出
	 */
	public <Ex extends Exception> RetOps<T> assertCode(int expect, Function<? super CommonResult<T>, ? extends Ex> func)
			throws Ex {
	...
	public <Ex extends Exception> RetOps<T> assertSuccess(Function<? super CommonResult<T>, ? extends Ex> func)
			throws Ex {
		return assertCode(SUCCESS.getCode(), func);
	}
	...
	public <U> RetOps<U> map(Function<? super T, ? extends U> mapper) {
		CommonResult<U> result = CommonResult.result(mapper.apply(original.getData()), original.getCode(),
				original.getMsg());
		return of(result);
	}
	 ...
	 */
	public <U> RetOps<U> mapIf(Predicate<? super CommonResult<T>> predicate, Function<? super T, ? extends U> mapper) {
		CommonResult<U> result = CommonResult.result(mapper.apply(original.getData()), original.getCode(),
				original.getMsg());
		return of(result);
	}
