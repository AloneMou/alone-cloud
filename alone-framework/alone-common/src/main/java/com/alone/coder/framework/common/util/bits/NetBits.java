...
public class NetBits {
    ...
    public static float getFloat(byte[] b, OffSet offset) {
        ...
        int i = ((b[off + 3] & 0xFF)) + ((b[off + 2] & 0xFF) << 8) + ((b[off + 1] & 0xFF) << 16) + ((b[off] & 0xFF) << 24);
    ...
    public static double getDouble(byte[] b, OffSet offset) {
        ...
        long j = ((b[off + 7] & 0xFFL)) + ((b[off + 6] & 0xFFL) << 8) + ((b[off + 5] & 0xFFL) << 16) + ((b[off + 4] & 0xFFL) << 24) + ((b[off + 3] & 0xFFL) << 32) + ((b[off + 2] & 0xFFL) << 40) + ((b[off + 1] & 0xFFL) << 48) + ((b[off] & 0xFFL) << 56);
    ...
    public static String getString(byte[] b, OffSet offset, int len, boolean hasEnd) {
        ...
        if (hasEnd) offset.setOff(off + len + 1);
        else offset.setOff(off + len);
    ...
    public static void putString(byte[] b, OffSet offset, String str, int len) {
        ...
        else System.arraycopy(ret, 0, b, off, ret.length);
    ...
    public static byte[] getBytes(byte[] b, int off, int len) {
        ...
        else System.arraycopy(b, off, bytes, 0, b.length - off);
    ...
    public static byte[] toBytes(String str, int len) {
        ...
        else System.arraycopy(buf, 0, bytes, 0, buf.length);
    ...
    public static String toHexString(byte[] b) {
        ...
        for (int i = 0; i < b.length; i++) {
            String str = Integer.toString((b[i] & 0xff) + 0x100, 16).substring(1).toUpperCase();
            strb.append(str);
        }
    ...
    public static byte[] readStream(InputStream input, int len) throws IOException {
        ...
        while (readcount < len) {
            ...
            if (-1 == ret) throw new EOFException("按长度读消息时,长度不够即到达流尾端");
            readcount += ret;
        }
        return buf;
    }
