package com.alone.coder.framework.redis.core;
 ...
 * <p>
 * 在 {@link Cacheable#cacheNames()} 格式为 "key#ttl" 时，# 后面的 ttl 为过期时间。 单位为最后一个字母（支持的单位有：d
 * 天，h 小时，m 分钟，s 秒），默认单位为 s 秒
 *
 * @author 芋道源码
 */
public class TimeoutRedisCacheManager extends RedisCacheManager {
	...
	@Override
	protected @NonNull RedisCache createRedisCache(@NonNull String name, RedisCacheConfiguration cacheConfig) {
		...
		if (cacheConfig != null) {
			...
			String ttlStr = StrUtil.subBefore(names[1], StrUtil.COLON, false); // 获得
																				// ttlStr
																				// 时间部分
